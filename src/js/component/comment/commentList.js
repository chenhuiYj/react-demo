        ...
        return (
            <div className="m-comment-list">
                ...
                {comment.commentList.map((item, index) => <div key={item.id}
                    ...
                    {this.state.isApply===item.id?<div><input placeholder="请输入回复内容" value={this.state.applyContent} onChange={this.handleBlur.bind(this)}/><a href="javascript:;" onClick={()=>{this.submitApply(index)}}>确定</a><a href="javascript:;" onClick={()=>{this.switchApply(-1)}}>取消</a></div>:<a href="javascript:;" onClick={()=>{this.switchApply(index)}}>回复</a>}
                    </div>)}
            </div>
        )
    }
