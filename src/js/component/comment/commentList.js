        ...
        return (
            <div className="m-comment-list">
                ...
                {comment.commentList.map((item, index) => <div key={item.id}
                    ...
                    {this.state.isApply===item.id?<div class="m-apply"><input placeholder="请输入回复内容" value={this.state.applyContent} onChange={this.handleBlur.bind(this)}/><a href="javascript:;" onClick={()=>{this.submitApply(index)}} style={{marginLeft:'10px'}}>确定</a><a href="javascript:;" onClick={()=>{this.switchApply(-1)}} style={{marginLeft:'10px'}}>取消</a></div>:<a href="javascript:;" onClick={()=>{this.switchApply(index)}} >回复</a>}
                    </div>)}
            </div>
        )
    }
